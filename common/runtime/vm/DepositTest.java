...
import org.sn.common.runtime.TVMTestResult;
...
/*  pragma solidity ^0.4.0;
  ...
  public void loopCallTest()
    ...
    byte[] aAddress = Wallet.generateContractAddress(aSn);
    byte[] bAddress = Wallet.generateContractAddress(bSn);
  ...
  public void loopCallTestOldVersion()
    ...
    Runtime runtime = TVMTestUtils.processTransactionAndReturnRuntime(aSn, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());

    Transaction bSn = TVMTestUtils.generateDeploySmartContractAndGetTransaction(
        contractB, address, bABI, bCode, value, fee, consumeUserResourcePercent, null);
    rootDeposit = DepositImpl.createRoot(manager);
    runtime = TVMTestUtils.processTransactionAndReturnRuntime(bSn, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
