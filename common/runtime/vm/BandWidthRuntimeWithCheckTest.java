...
import org.sn.common.runtime.RuntimeImpl;
import org.sn.common.runtime.TVMTestUtils;
import org.sn.common.runtime.vm.program.invoke.ProgramInvokeFactoryImpl;
...
import org.sn.core.capsule.ReceiptCapsule;
...
import org.sn.protos.Protocol.Transaction.Result.contractResult;
import org.sn.protos.Protocol.Transaction.raw;
...
public class BandWidthRuntimeWithCheckTest {
  ...
  public void testSuccess() {
    try {
      ...
      TransactionCapsule snCap = new TransactionCapsule(transaction);
      TransactionTrace trace = new TransactionTrace(snCap, dbManager);
      dbManager.consumeBandwidth(snCap, trace);
    ...
    try {
      ...
      Transaction transaction = Transaction.newBuilder().setRawData(raw.newBuilder().addContract(
          Contract.newBuilder().setParameter(Any.pack(triggerContract))
              .setType(ContractType.TriggerSmartContract)).setFeeLimit(1000000000)).build();
      TransactionCapsule snCap = new TransactionCapsule(transaction);
      snCap.setResultCode(contractResult.SUCCESS);
      TransactionTrace trace = new TransactionTrace(snCap, dbManager);
      dbManager.consumeBandwidth(snCap, trace);
      long bandWidth = snCap.getSerializedSize() + Constant.MAX_RESULT_SIZE_IN_TX;
      BlockCapsule blockCapsule = null;
      DepositImpl deposit = DepositImpl.createRoot(dbManager);
      Runtime runtime = new RuntimeImpl(trace, blockCapsule, deposit,
          new ProgramInvokeFactoryImpl());
