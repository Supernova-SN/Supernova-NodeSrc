...
import org.sn.common.runtime.vm.DataWord;
...
import org.sn.common.utils.ByteString;
...
public class ProgramResultTest {
  ...
  public void successAndFailResultTest()
    ...
    TransactionTrace traceSuccess = TVMTestUtils
        .processTransactionAndReturnTrace(sn1, deposit, null);
    ...
    checkTransactionInfo(traceSuccess, sn1, null, internalTransactionsList);
    ...
    byte[] triggerData2 = TVMTestUtils.parseABI("transfer(address,bool)",
        params);
    Transaction sn2 = TVMTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), aContract,
            triggerData2, 0, 100000000);
    TransactionTrace traceFailed = TVMTestUtils
        .processTransactionAndReturnTrace(sn2, deposit, null);
    runtime = traceFailed.getRuntime();
    byte[] bContract2 = Wallet
        .generateContractAddress(new TransactionCapsule(sn2).getTransactionId().getBytes(), 0);
    ...
    checkTransactionInfo(traceFailed, sn2, null, internalTransactionsListFail);
  }
  ...
  public void suicideResultTest()
    ...
    byte[] triggerData1 = TVMTestUtils.parseABI("suicide(address)",
        params);
    Transaction sn = TVMTestUtils
        .generateTriggerSmartContractAndGetTransaction(Hex.decode(OWNER_ADDRESS), suicideContract,
            triggerData1, 0, 100000000);
    TransactionTrace trace = TVMTestUtils.processTransactionAndReturnTrace(sn, deposit, null);
    ...
    Assert.assertEquals(internalTransactionsList.get(0).isRejected(), false);
    checkTransactionInfo(trace, sn, null, internalTransactionsList);
  }
  ...
  public void checkTransactionInfo(TransactionTrace trace, Transaction sn, BlockCapsule block,
      List<InternalTransaction> internalTransactionsList) {
    TransactionInfoCapsule snInfoCapsule = TransactionInfoCapsule
        .buildInstance(new TransactionCapsule(sn), null, trace);
    List<Protocol.InternalTransaction> internalTransactionListFromProtocol = snInfoCapsule
