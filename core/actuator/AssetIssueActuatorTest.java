...
import org.sn.core.config.Parameter;
...
public class AssetIssueActuatorTest {
  ...
  private static final String NAME = "sn-my";
  private static final long TOTAL_SUPPLY = 10000L;
  private static final int SN_NUM = 10000;
  ...
  public void SameTokenNameCloseAssetIssueSuccess() {
    ...
    try {
      ...
      Assert.assertEquals(SN_NUM, assetIssueCapsule.getSnNum());
      ...
      Assert.assertEquals(SN_NUM, assetIssueCapsuleV2.getSnNum());
  ...
  @Test
  /*
    Sn num must greater than zero.Else can't asset issue and balance do not change.
   */
  public void negativeSnNumTest() {
    long nowTime = new Date().getTime();
    Any contract = Any.pack(
        Contract.AssetIssueContract.newBuilder()
            .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)))
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertTrue("SnNum must greater than 0!".equals(e.getMessage()));
      AccountCapsule owner =
          dbManager.getAccountStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
      AssetIssueCapsule assetIssueCapsule =
          dbManager.getAssetIssueStore().get(ByteArray.fromString(NAME));
  ...
  @Test
  public void assetIssueSNNameTest() {
    dbManager.getDynamicPropertiesStore().saveAllowSameTokenName(1);
    Any contract = Any.pack(Contract.AssetIssueContract.newBuilder()
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)))
        .setName(ByteString.copyFromUtf8("SN"))
        .setTotalSupply(TOTAL_SUPPLY)
        .setSnNum(SN_NUM).setNum(NUM)
        .setStartTime(startTime)
        .setEndTime(endTime)
    ...
    try {
      ...
      Assert.assertTrue(e instanceof ContractValidateException);
      Assert.assertEquals("assetName can't be sn", e.getMessage());
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
    } finally {
      dbManager.getAssetIssueStore().delete(ByteArray.fromString(NAME));
