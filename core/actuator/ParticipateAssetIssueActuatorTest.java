...
import org.sn.core.capsule.AssetIssueCapsule;
...
public class ParticipateAssetIssueActuatorTest {
  ...
  private static final int SN_NUM = 2;
  private static final int NUM = 2147483647;
  private static final int VOTE_SCORE = 2;
  private static final String DESCRIPTION = "SN";
  ...
  public void sameTokenNameCloseRightAssetIssue() {
    ...
    try {
      ...
      //V1
      Assert.assertEquals(owner.getAssetMap().get(ASSET_NAME).longValue(), (1000L) / SN_NUM * NUM);
      Assert.assertEquals(toAccount.getAssetMap().get(ASSET_NAME).longValue(),
          TOTAL_SUPPLY - (1000L) / SN_NUM * NUM);
      //V2
  ...
  public void sameTokenNameCloseExchangeDevisibleTest() {
    ...
    try {
      ...
      Assert.assertEquals(owner.getAssetMap().get(ASSET_NAME).longValue(), (999L * NUM) / SN_NUM);
      Assert.assertEquals(
          toAccount.getAssetMap().get(ASSET_NAME).longValue(),
          TOTAL_SUPPLY - (999L * NUM) / SN_NUM);
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
  ...
  public void assetNameTest() {
    ...
    try {
      ...
      Assert.assertEquals(owner.getBalance(), OWNER_BALANCE - 1000);
      Assert.assertEquals(toAccount.getBalance(), TO_BALANCE + 1000);
      Assert.assertEquals(owner.getAssetMap().get(assetName).longValue(), (1000L) / SN_NUM * NUM);
  ...
  /**
   * SameTokenName close, not enough sn
   */
  @Test
  public void sameTokenNameCloseNotEnoughSnTest() {
    initAssetIssue(dbManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() - 1000,
        dbManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
    // First, reduce the owner sn balance. Else can't complete this test case.
  ...
  /**
   * SameTokenName open, not enough sn
   */
  @Test
  public void sameTokenNameOpenNotEnoughSnTest() {
    dbManager.getDynamicPropertiesStore().saveAllowSameTokenName(1);
    initAssetIssue(dbManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() - 1000,
        dbManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
    // First, reduce the owner sn balance. Else can't complete this test case.
  ...
  public void sameTokenNameCloseMultiplyOverflowTest() {
    initAssetIssue(dbManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() - 1000,
        dbManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
    // First, increase the owner sn balance. Else can't complete this test case.
    AccountCapsule owner = dbManager.getAccountStore().get(ByteArray.fromHexString(OWNER_ADDRESS));
  ...
  /**
   * SameTokenName close, exchangeAmount <= 0 sn, throw exception
   */
  @Test
  ...
  /**
   * SameTokenName open, exchangeAmount <= 0 sn, throw exception
   */
  @Test
