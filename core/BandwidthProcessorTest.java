...
import org.sn.core.db.BandwidthProcessor;
...
public class BandwidthProcessorTest {
  ...
  public void testCreateNewAccount() throws Exception {
    ...
    TransactionCapsule sn = new TransactionCapsule(transferAssetContract);
    ...
    org.sn.protos.Protocol.Transaction.Contract contract = org.sn.protos.Protocol.Transaction.Contract
        .newBuilder()
    ...
    long bytes = sn.getSerializedSize();
  ...
  public void testFree() throws Exception {
    ...
    TransactionCapsule sn = new TransactionCapsule(contract);
    ...
    TransactionTrace trace = new TransactionTrace(sn, dbManager);

    dbManager.consumeBandwidth(sn, trace);
...
//    Assert.assertEquals(101220L, ret.getFee());
  ...
  public void sameTokenNameCloseConsumeSuccess() {
    ...
    long byteSize = sn.getInstance().toBuilder().clearRet().build().getSerializedSize() +
        Constant.MAX_RESULT_SIZE_IN_TX;

    BandwidthProcessor processor = new BandwidthProcessor(dbManager);

    try {
      processor.consume(sn, trace);
  ...
  public void sameTokenNameCloseTransferToAccountNotExist() {
    ...
    TransactionCapsule sn = new TransactionCapsule(contract, dbManager.getAccountStore());
    TransactionTrace trace = new TransactionTrace(sn, dbManager);
